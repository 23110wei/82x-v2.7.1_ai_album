...
# makecode生成的带版本名文件里,日期取自txw82xcore.bin(预编译,日期恒定),
# 会误导为"固件没变";改名为真实构建时间的名字(生成时刻,分钟级)
STAMP=$(date +%Y.%m.%d_%H%M)
for f in txw82xApp_v*_app-*.bin; do
	[ -e "$f" ] || continue
	case "$f" in
		*_app-0_$STAMP.bin) continue ;;  # 已是目标名(极端同名)
	esac
	rm -f txw82xApp_v*_app-*_${STAMP}.bin 2>/dev/null
	mv -f "$f" "ai_album_${STAMP}.bin"
	echo "renamed $f -> ai_album_${STAMP}.bin"
done
