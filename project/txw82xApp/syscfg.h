...
struct sys_config {
    ...
    /* 相框应用配置(追加区,只能往后追加不能改前段;0xFF=未设置) */
    uint8  album_lang;      /* ai_album_language_t: 0=EN 1=zh-CN 2=ja-JP */
    uint8  album_bl_pct;    /* 背光亮度百分比 20/40/60/80/100 */
    uint8  album_bl_marker; /* 0xB7=背光字段有效 */
    uint8  album_ble;      /* 蓝牙开关:0xFF=未设置(默认关) 0=off 1=on */
    uint32 album_wx_lat;   /* 天气位置纬度x1e4(0xFFFFFFFF=未设置,默认深圳) */
    uint32 album_wx_lon;   /* 天气位置经度x1e4 */
    char   album_wx_city[24]; /* 天气位置城市名 */
    uint8  album_vol;      /* 音量百分比0/25/50/75/100,0xFF=未设置(默认100) */
};
