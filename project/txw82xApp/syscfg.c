...
void syscfg_default()
{
    ...
    sys_cfgs.album_lang      = 0xFF;//未设置,UI用默认语言
    sys_cfgs.album_bl_pct    = 0xFF;//未设置,亮度用默认档
    sys_cfgs.album_bl_marker = 0xFF;
    sys_cfgs.album_ble      = 0xFF;
    sys_cfgs.album_wx_lat   = 0xFFFFFFFF;//天气位置未设置,用默认(深圳)
    sys_cfgs.album_wx_lon   = 0xFFFFFFFF;
    sys_cfgs.album_wx_city[0] = ' ';
    sys_cfgs.album_vol       = 0xFF; // 鉋速未设置(默认100%)

#ifdef PHOTO_FRAME_WIFI_DEFAULT_SSID
    /* 开发调试:默认STA直连指定热点(烧录/重建默认配置后免设置),
     * 量产前删除本段或关闭PHOTO_FRAME_WIFI_DEFAULT_SSID宏 */
    sys_cfgs.wifi_mode = WIFI_MODE_STA;
    sys_cfgs.dhcpc_en = 1;
    sys_cfgs.key_mgmt = 2; //WPA/WPA2-PSK(与设置页写入值一致)
    os_snprintf((char *)sys_cfgs.ssid, sizeof(sys_cfgs.ssid), "%s",
                PHOTO_FRAME_WIFI_DEFAULT_SSID);
    os_strcpy(sys_cfgs.passwd, PHOTO_FRAME_WIFI_DEFAULT_PASSWD);
    wpa_passphrase(sys_cfgs.ssid, (char *)sys_cfgs.passwd, sys_cfgs.psk);
    os_printf("syscfg default: preset STA ssid=%s\r\n", sys_cfgs.ssid);
#endif
}
