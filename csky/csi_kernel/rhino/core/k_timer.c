...
void krhino_timer_dump(void)
{
    k_timer_queue_cb cb;
    cb.timer  = NULL;
    cb.cb_num = TIMER_CMD_DUMP;
    krhino_buf_queue_send(&g_timer_queue, &cb, sizeof(k_timer_queue_cb), 0);
}
...
static void cmd_proc(k_timer_queue_cb *cb, uint8_t cmd)
{
    ...
    switch (cmd) {
        ...
        case TIMER_CMD_DUMP:{
                ...
                klist_t     *end;
                ktimer_t    *t;
                printf("Timer Dump, tick:%llu\r\n", krhino_sys_tick_get());
                start = end = &g_timer_head;
                for (q = start->next; q != end; q = q->next) {
                    t = krhino_list_entry(q, ktimer_t, timer_list);
                    printf("   timer %p match=%lld, remain:%llu, round_ticks:%llu, cb:%p, cb_arg:%p, next:%p\r\n", 
                           t, t->match, t->remain, t->round_ticks, t->cb, t->timer_cb_arg, q->next);
                }
            }
            break;
        case TIMER_CMD_DEL:
