...
void krhino_task_dump(ktask_t *task, void* stack, uint32_t lr, uint32_t pc)
{
    int i = 0;
...
__lable:
    hgprintf("\0015Task:%s\r\n", task->task_name);
    hgprintf("\0015    task_state: %d\r\n", task->task_state);
    hgprintf("\0015    stack_size: %d\r\n", task->stack_size * 4);
    hgprintf("\0015    task_stack: 0x%08x,0x%08x,0x%08x\r\n", (uint32_t)task->task_stack_base, (uint32_t)p, (uint32_t)(task->task_stack_base + task->stack_size));
    if(task == g_active_task[0]){
        if(lr && pc){
            hgprintf("\0015    task_lr   : %p\r\n", lr);
            hgprintf("\0015    task_pc   : %p\r\n", pc);
        }else{
