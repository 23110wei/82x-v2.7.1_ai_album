...
extern void krhino_task_dump(ktask_t *task, void* stack, uint32_t lr, uint32_t pc);
...
void csi_kernel_task_dump(k_task_handle_t task_handle, void* stack, uint32_t lr, uint32_t pc)
{
    krhino_task_dump(task_handle, stack, lr, pc);
}
...
uint32_t csi_kernel_task_runtime(struct os_task_info *tsk_rumtime, uint32_t count, uint32_t diff_tick)
{
#if (RHINO_CONFIG_SYSTEM_STATS > 0)
    uint32_t ret = 0;
    klist_t *taskhead;
    ...
    for (tmp = taskhead->next; tmp != taskend && ret < count; tmp = tmp->next) {
        ...
        tsk_rumtime[ret].time = (task->runtime*1000)/diff_tick;
