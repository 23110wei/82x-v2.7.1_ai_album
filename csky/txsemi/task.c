...
extern uint32_t csi_kernel_task_runtime(struct os_task_info *tsk_rumtime, uint32_t count, uint32_t diff_tick);
extern void csi_kernel_task_dump(k_task_handle_t task_handle, void* stack, uint32_t lr, uint32_t pc);
...
{
    return csi_kernel_task_runtime(tsk_times, count, diff_tick);
}
...
{
    csi_kernel_task_dump(hdl, stack, 0, 0);
}
