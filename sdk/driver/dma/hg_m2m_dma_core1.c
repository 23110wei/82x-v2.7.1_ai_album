...
int32 hg_m2m_dma_resume(struct dev_obj *dev)
{
    ...
    {
        // dma->hw->dma_ch[i].DMA_CON  = 0x00;
        // dma->hw->dma_ch[i].DMA_SAIE = HG_M2M_DMA_SAIE_TCP_PENDING;
        irq_enable(dma->irq_num[i]);
        hg_m2m_dma_free_ch(dma, i);
    }
    dma->suspend = 0;
...
__init int32 hg_m2m_dma_dev_attach(uint32 dev_id, struct mem_dma_dev *p_dma)
{
    ...
    void *irq_handler[HG_M2M_DMA_NUM] = { hg_m2m2_dma_irq_handler};
