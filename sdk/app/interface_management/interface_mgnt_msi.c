...
{
    /* ai_album全量UI页面构建调用链较深(AI对话页实测4KB栈溢出),
     * 栈扩到16KB并放PSRAM(参考ai_album工程的LVGL任务做法) */
    void *lvgl_stack = os_malloc_psram(16 * 1024);
    g_lvgl_hdl = os_task_create("gui_thread", lvgl_run_msi, (void *) ui_start,
                                OS_TASK_PRIORITY_ABOVE_NORMAL, 0,
                                lvgl_stack, 16 * 1024);
}