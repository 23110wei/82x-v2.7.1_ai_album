...
void deliver_frame_to_stream( struct frame *f, void *d )
{
	...
	{
		/* destination is waiting for frame ?*/
		if( ! dest->waiting ) {
...
void del_dest( struct stream_destination *dest )
{
	if(dest)
	{
		if(dest->next) dest->next->prev = dest->prev;
		if(dest->prev) dest->prev->next = dest->next;
		else dest->stream->dest_list = dest->next;
		free(dest);
	}
}
...
int find_stream( const char *name )
{
	struct stream *s;
