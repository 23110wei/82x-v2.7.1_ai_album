...
void new_rtsp_location( const char *path, char *realm, char *username, char *password,
			open_func open, set_track_fun set_track, void *private )
{
...
static int handle_DESCRIBE( struct req *req )
{
	...
	if( ! ( loc = find_rtsp_location( req->req->sl.req.uri, path, NULL ) ) ||
	...
	if( loc->set_track )
	{
		if( loc->set_track( loc->private ) )
		{
			_os_printf("set track error\n");
			rtsp_send_error( req, 404, "Not Found" );
			return 0;
		}
	}
