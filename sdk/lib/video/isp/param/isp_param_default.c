...
#define  DEFAULT_MASTER_SENSOR	1
#define  DEFAULT_SLAVE0_SENSOR	0
#define  DEFAULT_SLAVE1_SENSOR	0


#if  DEFAULT_MASTER_SENSOR
/* ============================================================
 *  DEFAULT Master 静态标定参数
 * ============================================================ */

/* ---------- Enable 参数 ---------- */
static const _ISP_FUNC default_master_enable = {
    .test_pattern_en    = 0,
    .dma_flush_en       = 1,
    .blc_en             = 1,
    .awb_en             = 1,
...
static const TYPE_HGISP_CFG_AWB default_master_awb = {
...
static const struct hgisp_cfg_ae default_master_ae = {
        .ae_manual_en              = 0,
        .luma_target               = 65, 
        .luma_weight_sum           = 610,
    ...
    .sensor_ae = {
        .dark_scene_target_lut = {50, 65},
        .dark_scene_bv_lut     = {47, 363},
        .hs_scene_limit_lut    = {65, 75},
        .hs_scene_bv_lut       = {363, 3022},
        .lowlight_lsb_bv_lut   = {47, 94, 195, 381, 781, 1636, 3225, 1e30},
        .lowlight_lsb_gain_lut = {16, 16,  16,  16,  16,   16,   16,   16}, 
    },
};

/* ---------- BLC ---------- */
static const _Sensor_BLC default_master_blc = {
    .gr = 256, 
    .gb = 256, 
    .r  = 256, 
    .b  = 256,
...
/* ---------- CCM ---------- */
static const _Sensor_CCM default_master_ccm = {
    480,  -95,  -64,
   -208,  456, -200,
    -16, -105,  520,
      0,    0,    0,
...
/*  DEFAULT Master Gamma 参数 */
const _Sensor_GAMMA_BV default_master_gamma = 
{
    .adj_by_bv  = 1,
...
/*  DEFAULT Master CSC 参数 */
const _Sensor_CSC default_master_csc = 
{
    ...
    .gamma_alpha_map       = (_Sensor_GAMMA_BV *)&default_master_gamma,
};

/* ---------- DPC ---------- */
static const _Sensor_DPC default_master_dpc = {
...
/* ---------- CE ---------- */
static const _Sensor_BV2COLENH default_master_ce = {
    .sensor_ce = {
...
/* ---------- SHARP ---------- */
static const _Sensor_SHARP default_master_sharp = {
    .filt_alpha      = 128,
    .shrink_thr      = 0  ,
    .filt_clip_hi    = 127,
    .filt_clip_lo    = 127,
...
/* ---------- NR ---------- */
static const _Sensor_CSUPP default_master_csupp[BV2CSUPP_ARRAY_NUM] = {
...
static const _Sensor_YUVNR default_master_yuvnr[BV2YUVNR_ARRAY_NUM] = {
// group is arranged from bright to dark
// group 0:
    {5,5,5,5,5,5,5,5,
    128,255,0},
...
static const _Sensor_BV2NR default_master_bv2nr[BV2RAWNR_ARRAY_NUM] = {
...
/* ---------- GIC ---------- */
 static const _Sensor_GIC default_master_gic = {
    .w_thres  = 14,
    .w_slope  = 16,
    .w_str    = 127,
    .mu_thres = 5,
...
/* ---------- LHS ---------- */
static const _Sensor_LHS_MAP default_master_lhs = {
    .local_hue_sat_map = {
...
static const TYPE_HGISP_CFG_WDR default_master_wdr = {
    .dynamic_gamma_en          = 0,
    .y_gamma_opt               = 0,
    .wdr_en                    = 0,
    .wdr_opt                   = 0,
...
/* ============================================================
 *  DEFAULT YGamma 曲线数据 (Master)
 * ============================================================ */
static const _Sensor_YGAMMA_MAP default_master_ygamma = {
    .local_ygamma_map = {
        {
        .bv = 200,
        .packed_lut = {
...
// 3组RGB Gamma LUT
static const unsigned int rgb_gamma_lut_group[3][64] =
{
    // ========== [0] Gamma2.4 ==========
    {
...
static const _Sensor_RGBGAMMA default_rgb_gamma_tbl = {
    .p_rgb_gamma_tbl = (unsigned int *)rgb_gamma_lut_group[1],
};

/* ============================================================
 *  DEFAULT LSC 数据 (Master)
 * ============================================================ */
static const unsigned int default_master_lsc_tbl[612] = {
    //R channel
...
static const _Sensor_LSC default_master_lsc = {
    .p_lsc_tbl = (unsigned int *)default_master_lsc_tbl,
};

#endif


const _Sensor_ISP_Init default_isp_param_init[ISP_SUPPORT_SENSOR_MAX_NUM] = 
{
#if  DEFAULT_MASTER_SENSOR
    {
        .p_func        = (_ISP_FUNC          *)&default_master_enable,
        .p_blc         = (_Sensor_BLC        *)&default_master_blc,
        .p_ccm         = (_Sensor_CCM        *)&default_master_ccm,
        .p_dpc         = (_Sensor_DPC        *)&default_master_dpc,
        .p_colenh      = (_Sensor_BV2COLENH  *)&default_master_ce, 
        .p_csc         = (_Sensor_CSC        *)&default_master_csc,
		.p_gamma_param = (_Sensor_GAMMA_BV   *)&default_master_gamma,
        .p_gic         = (_Sensor_GIC        *)&default_master_gic,
        .p_csupp       = (_Sensor_CSUPP      *)default_master_csupp,
        .p_sharp       = (_Sensor_SHARP      *)&default_master_sharp,
        .p_yuvnr       = (_Sensor_YUVNR      *)default_master_yuvnr,
        .p_bv2nr       = (_Sensor_BV2NR      *)default_master_bv2nr,
        .p_lsc         = (_Sensor_LSC        *)&default_master_lsc,
        .p_lhs         = (_Sensor_LHS_MAP    *)&default_master_lhs,
        .p_ygamma      = (_Sensor_YGAMMA_MAP *)&default_master_ygamma,
        .p_rgb_gamma   = (_Sensor_RGBGAMMA   *)&default_rgb_gamma_tbl,
        .p_awb         = (TYPE_HGISP_CFG_AWB *)&default_master_awb,
        .p_ae          = (TYPE_HGISP_CFG_AE  *)&default_master_ae,
        .p_wdr         = (TYPE_HGISP_CFG_WDR *)&default_master_wdr,
    },
#endif
#if  DEFAULT_SLAVE0_SENSOR
    {
        .p_func        = (_ISP_FUNC          *)&default_master_enable,
        .p_blc         = (_Sensor_BLC        *)&default_master_blc,
        .p_ccm         = (_Sensor_CCM        *)&default_master_ccm,
        ...
        // .p_func   = (_ISP_FUNC *)&default_slave0_enable,
        // .p_blc    = (_Sensor_BLC *)&default_slave0_blc,
        // .p_ccm    = (_Sensor_CCM *)&default_slave0_ccm,
        // .p_awb    = (TYPE_HGISP_CFG_AWB *)&default_slave0_awb,
        // .p_ae     = (TYPE_HGISP_CFG_AE *)&default_slave0_ae,
        // .p_dpc    = (_Sensor_DPC *)&default_slave0_dpc,
        // .p_csc    = (_Sensor_CSC *)&default_slave0_csc,
        // .p_gic    = (_Sensor_GIC *)&default_slave0_gic,
        // .p_csupp  = (_Sensor_CSUPP *)&default_slave0_csupp,
        // .p_sharp  = (_Sensor_SHARP *)&default_slave0_sharp,
        // .p_yuvnr  = (_Sensor_YUVNR *)&default_slave0_yuvnr,
        // .p_colenh = (_Sensor_BV2COLENH *)&default_slave0_ce,
        // .p_bv2nr  = (_Sensor_BV2NR *)default_slave0_bv2nr,
        // .p_lhs    = (_Sensor_LHS_MAP *)&default_slave0_lhs,
        // .p_lsc    = (_Sensor_LSC *)&default_slave0_lsc,
        // .p_ygamma = (_Sensor_YGAMMA_MAP *)&default_slave0_ygamma,
        // .p_wdr    = (TYPE_HGISP_CFG_WDR *)&default_slave0_wdr,
    },
#endif
#if  DEFAULT_SLAVE1_SENSOR
    {
        .p_func        = (_ISP_FUNC          *)&default_master_enable,
        .p_blc         = (_Sensor_BLC        *)&default_master_blc,
        .p_ccm         = (_Sensor_CCM        *)&default_master_ccm,
        ...
        // .p_func   = (_ISP_FUNC *)&default_slave1_enable,
        // .p_blc    = (_Sensor_BLC *)&default_slave1_blc,
        // .p_ccm    = (_Sensor_CCM *)&default_slave1_ccm,
        // .p_awb    = (TYPE_HGISP_CFG_AWB *)&default_slave1_awb,
        // .p_ae     = (TYPE_HGISP_CFG_AE *)&default_slave1_ae,
        // .p_dpc    = (_Sensor_DPC *)&default_slave1_dpc,
        // .p_csc    = (_Sensor_CSC *)&default_slave1_csc,
        // .p_gic    = (_Sensor_GIC *)&default_slave1_gic,
        // .p_csupp  = (_Sensor_CSUPP *)&default_slave1_csupp,
        // .p_sharp  = (_Sensor_SHARP *)&default_slave1_sharp,
        // .p_yuvnr  = (_Sensor_YUVNR *)&default_slave1_yuvnr,
        // .p_colenh = (_Sensor_BV2COLENH *)&default_slave1_ce,
        // .p_bv2nr  = (_Sensor_BV2NR *)default_slave1_bv2nr,
        // .p_lhs    = (_Sensor_LHS_MAP *)&default_slave1_lhs,
        // .p_lsc    = (_Sensor_LSC *)&default_slave1_lsc,
        // .p_ygamma = (_Sensor_YGAMMA_MAP *)&default_slave1_ygamma,
        // .p_wdr    = (TYPE_HGISP_CFG_WDR *)&default_slave1_wdr,
    },
#endif
};
