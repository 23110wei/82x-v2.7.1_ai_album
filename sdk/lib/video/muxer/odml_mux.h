...
typedef struct
{
  ...
  */
  ...
  uint32 avi1framecnt;      // video frames stored in the first RIFF 'AVI ' list
  uint32 vframecnt;         // video frame count total
  uint32 aframecnt;         // auds frame count total
  uint32 audio_bytes_total; // total PCM payload bytes written
  uint32 aidx_bytes;        // PCM payload bytes in current audio std index block
