...
#undef  log_e
#endif
#define log_e(...)                     FAL_PRINTF("[Error/FAL] (%s:%d) ", __func__, __LINE__);FAL_PRINTF(__VA_ARGS__);FAL_PRINTF("\033[0m\n")

/* info level log */
#ifdef  log_i
#undef  log_i
#endif
#define log_i(...)                     FAL_PRINTF("[I/FAL] ");                                FAL_PRINTF(__VA_ARGS__);FAL_PRINTF("\033[0m\n")
...
struct fal_flash_dev
{
    ...
    /* user custom data pointer, such as device handle,hugeic added*/
    void *user_data;
};
