...
static struct fal_partition partition_table_runtime[sizeof(partition_table_def) / sizeof(partition_table_def[0])];
...
static int check_and_update_part_cache(const struct fal_partition *table, size_t len)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if (table[i].offset + table[i].len > flash_dev->len)
        {
            log_e("Initialize failed! Partition(%s) offset+len(0x%lx) exceeds flash size(0x%x).",
                    table[i].name, table[i].offset + table[i].len, flash_dev->len);
            partition_table_len = 0;
...
int fal_partition_init(void)
{
    ...
    memcpy(partition_table_runtime, partition_table_def,
           sizeof(partition_table_def));

    if (fal_partition_table_update(partition_table_runtime,
                                  partition_table_len) < 0)
    {
        log_e("Partition table update hook failed!");
        return 0;
    }

    partition_table = partition_table_runtime;
#else
