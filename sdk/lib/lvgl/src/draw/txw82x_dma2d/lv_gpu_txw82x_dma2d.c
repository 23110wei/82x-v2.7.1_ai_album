...
/* 逐笔明细日志：默认关闭，打开会刷屏 */
#if TXW82X_DMA2D_VERBOSE
    #define DMA2D_VLOG(fmt, ...)  os_printf("[DMA2D] " fmt "\r\n", ##__VA_ARGS__)
#else
    #define DMA2D_VLOG(fmt, ...)
#endif
...
// Blend 入口
#if TXW82X_DMA2D_DEBUG
/* 临时统计：每 5 秒打印一次 DMA2D 命中率（测完删除本函数与调用） */
static void dma2d_stats_report(void)
{
    static uint64 last_jiffies = 0;
    uint64 now = os_jiffies();

    if (last_jiffies == 0) {
        last_jiffies = now;
        return;
    }
    if (now - last_jiffies < 5000) {
        return;
    }
    last_jiffies = now;
    lv_gpu_txw82x_dma2d_print_stats();
}
#endif
                       ...
                       lv_color_t color)
{
    ...
    if (stat_fill_calls % 50 == 1) {
        DMA2D_VLOG("fill #%u: %dx%d dst=%08X stride=%d dst_h=%d status=%d", stat_fill_calls, area_w, area_h,
                      ...
                      lv_coord_t src_stride, lv_opa_t opa)
{
    ...
    if (stat_copy_calls % 200 == 1) {
        DMA2D_VLOG("copy #%u: %dx%d src=%08X(s=%d) -> dst=%08X(s=%d)", stat_copy_calls, dest_w, dest_h,
                      ...
                      lv_coord_t src_stride, lv_opa_t opa)
{
    ...
    if (stat_mix_calls % 200 == 1) {
        DMA2D_VLOG("mix #%u: %dx%d opa=%d src=%08X(s=%d) dst=%08X(s=%d)", stat_mix_calls, dest_w, dest_h, (int)opa,
                  (uint32_t)src_buf, src_stride, (uint32_t)dest_buf, dest_stride);
    }
#endif
                           ...
                           lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa)
{
    ...
    if (stat_mix_fill_calls % 200 == 1) {
        DMA2D_VLOG("mix_fill #%u: %dx%d color=%08X opa=%d -> dst=%08X(s=%d)", 
