#!/usr/bin/env python3
"""通过 ApplicationLoader TCP 协议写入 Loader OTA 镜像。

成功提交后不会发送重启命令；请先检查设备日志，再手动重启。
"""
...
def receive_exact(connection: socket.socket, size: int) -> bytes:
    ...
    while len(data) < size:
        ...
        if not chunk:
            raise UpgradeError(f"连接已关闭，只接收到 {len(data)}/{size} 字节")
        data.extend(chunk)
    return bytes(data)


def encode_version(version: str) -> bytes:
    """编码 16 字节、以 NUL 结尾的 Loader 版本字段。"""
...
def _format_progress(sent_bytes: int, total_bytes: int, started_at: float) -> str:
...
def wait_for_result(connection: socket.socket) -> bytes:
    ...
    while len(response) < maximum_size:
        ...
        if not chunk:
            if not response:
                raise UpgradeError("连接在返回升级结果前已关闭")
            raise UpgradeError("设备返回的升级结果不完整")
        response.extend(chunk)
        nul_index = response.find(b"\x00")
        if nul_index >= 0:
            result = bytes(response[: nul_index + 1])
            if result == RESULT_OK:
                return RESULT_OK
    ...
    printable = bytes(response).decode("ascii", errors="replace")
    raise UpgradeError(f"设备返回未知或未终止的结果: {printable!r}")
...
def _validate_inputs(
    ...
    if (
        ...
        or not math.isfinite(result_timeout)
        or result_timeout <= 0
    ):
        raise UpgradeError("连接超时和结果超时必须为有限正数")
    return file_size
...
def upgrade_loader(
    ...
    """连接设备、写入 Loader 镜像并等待提交结果。"""
    file_size = _validate_inputs(
        firmware_path, port, connect_timeout, result_timeout, chunk_size
    )
    ...
    print(f"Loader 镜像: {firmware_path.resolve()}")
    print(f"镜像版本: {version}")
    print(f"镜像大小: {file_size} 字节")
    ...
    with connection:
        try:
            ...
            ack_fields = struct.unpack(
                TCP_HEADER_FORMAT, receive_exact(connection, TCP_HEADER_SIZE)
            )
            ...
            with firmware_path.open("rb") as firmware:
                while True:
                    ...
                    print(
                        _format_progress(sent_bytes, file_size, started_at),
                        end="",
                        flush=True,
                    )
            print()
            ...
            if wait_for_result(connection) == RESULT_ERROR:
                raise UpgradeError("设备拒绝或写入 Loader 镜像失败")
        except socket.timeout as error:
            raise UpgradeError("等待设备响应超时") from error
        except OSError as error:
            raise UpgradeError(f"TCP 通信失败: {error}") from error

    print("Loader 镜像写入并提交成功。")
    print("请先确认设备日志无 error=-16，再由用户手动重启设备。")


def parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="通过 ApplicationLoader TCP 协议写入 Loader OTA 镜像（不自动重启）。"
    )
    parser.add_argument("host", help="设备 IPv4 地址或主机名")
    parser.add_argument("firmware", type=Path, help="Loader 镜像路径")
    parser.add_argument(
        "--port", type=int, default=DEFAULT_PORT, help="TCP 端口，默认 20203"
    )
    parser.add_argument(
        "--version",
        default="Loader",
        help="版本字符串，UTF-8 编码后最多 15 字节，默认 Loader",
    )
    parser.add_argument(
        "--connect-timeout",
        type=float,
        default=DEFAULT_CONNECT_TIMEOUT,
        help="连接和 ACK 超时秒数，默认 10",
    )
    parser.add_argument(
        "--result-timeout",
        type=float,
        default=DEFAULT_RESULT_TIMEOUT,
        help="等待写入结果的超时秒数，默认 30",
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
        default=DEFAULT_CHUNK_SIZE,
        help="发送分块字节数，默认 4096",
    )
