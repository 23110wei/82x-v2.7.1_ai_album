...
extern struct hgusb20_dev usb20_host;

static struct hgusb20_dev *hg_usb_detect_get_device(void)
{
    struct hgusb20_dev *p_dev = (struct hgusb20_dev *)dev_get(HG_USBDEV_DEVID);

    if(p_dev == NULL)
    {
        p_dev = (struct hgusb20_dev *)dev_get(HG_USBHOST_DEVID);
    }
...
{
    uint32 mask = 0;
    struct usb_device *p_dev = (struct usb_device *)dev_get(HG_USBHOST_DEVID);
...
static int32 hg_usb_connect_detect_work(struct os_work *work)
{
    ...
    struct hgusb20_dev *p_dev = hg_usb_detect_get_device();
    if(p_dev == NULL)
    {
